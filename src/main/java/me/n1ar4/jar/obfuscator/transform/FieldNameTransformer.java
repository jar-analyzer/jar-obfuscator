...
public class FieldNameTransformer {
    ...
    public static void transform(CustomClassLoader loader) {
        for (Map.Entry<String, String> entry : ObfEnv.classNameObfMapping.entrySet()) {
            ...
            logger.debug("混淆字段名进行中 {} -> {}", newClassPath.toAbsolutePath());
            ...
            try {
                TransformerUtil.transformClass(newClassPath, loader, FieldNameVisitor::new);
            } catch (Exception ex) {
                logger.error("transform error: {}", ex.toString());
                throw new IllegalStateException("field name transform failed: " + newName, ex);
            }
        }
